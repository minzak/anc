...
# Определяем максимальную ширину для каждого столбца (можно настроить по необходимости)
col_widths = [16, 14, 18, 18, 16]  # ширина для каждого столбца

# Цикл по всем pdf в директории
for filename in os.listdir(os.path.join(Stadiu, directory)):
        ...
        for page in doc:
            ...
            for line in tab.extract():  # print cell text for each row
                ...
                if ID and ID[0].isdigit():
                    ...
                    # Заполняем недостающие столбцы пустыми значениями
                    extended_line = filtered_line + [''] * (max_columns - len(filtered_line))

                    # Форматируем строку с разделителями и выравниванием
                    #formatted_row = "".join(f"{C_INFO}[{C_RESET}{str(cell) if cell is not None else '':>{col_widths[i]}}{C_INFO}]{C_RESET} " for i, cell in enumerate(extended_line))
                    formatted_row = "".join(f"{C_INFO}[{C_RESET}{str(cell):>{col_widths[i]}}{C_INFO}]{C_RESET} " for i, cell in enumerate(extended_line))

                    if size_line == 5:
                        # Строка распозналась корректно, все 5 полей
                        print(f"{'Algo '}{C_SUCCESS}5{C_RESET} tabs.".ljust(31), formatted_row)
                        ...
                        print(f"{'Algo '}{C_SUCCESS}4{C_RESET} tabs.".ljust(31), formatted_row)
                        ...
                        if '/' in string and vali_date(string.split('/')[-1]):
                            ...
                            print(f"{'Algo '}{C_SUCCESS}3{C_RESET} tabs. {C_INFO}Old{C_RESET}      ".ljust(31), formatted_row)
                            ...
                            print(f"{'Algo '}{C_SUCCESS}3{C_RESET} tabs. {C_INFO}No Termen{C_RESET}".ljust(31), formatted_row)
                            ...
                            print(f"{'Algo '}{C_SUCCESS}3{C_RESET} tabs.".ljust(31), formatted_row)
                        ...
                        print(f"{'Algo '}{C_SUCCESS}2{C_RESET} tabs.".ljust(31), formatted_row)
