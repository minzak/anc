...
def main():
    # Gather all stadiu PDFs across every dated snapshot directory, then keep
    # only those not yet processed (tracked in .state/stadiu.json). Each download
    # run drops changed snapshots into a NEW dated dir, so an unseen path == new
    # data. Unlike the old "last directory only" logic, this does not lose data
    # when more than one downloader run happened since the last parse.
    from incremental import FileState
    # New dated path implies new file; no need to hash large snapshot PDFs.
    state = FileState('stadiu', use_hash=False)
    ...
    for d in sorted(os.listdir(PDF_DIR)):
        ...
        for filename in sorted(os.listdir(dir_path)):
            if filename.lower().endswith('.pdf'):
                candidates.append(os.path.abspath(os.path.join(dir_path, filename)))

    files = state.new_files(candidates)
    print(f"{C_SUCCESS}Found {len(files)} new PDF files (of {len(candidates)} total){C_RESET}")

    # Parsing in multiprocessing; mark each file in the parent AS IT COMPLETES
    # (Pool children must not touch the sidecar). This makes the run resumable:
    # a crash leaves already-finished files marked, so a re-run skips them.
    from multiprocessing import Pool
    agg = {'files': 0, 'parsed': 0, 'header': 0, 'boiler': 0, 'unknown': 0}
    with Pool(processes=4) as pool:
        for r in pool.imap_unordered(process_pdf, files):
            if not isinstance(r, dict):
                continue
            state.mark(r['file'])   # persist immediately (crash-safe)
